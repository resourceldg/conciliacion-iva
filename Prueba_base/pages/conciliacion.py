...
========================
Cruza Listado IVA Compras (Colppy) con Mis Comprobantes Recibidos (ARCA).
set_page_config y CSS se aplican en app.py.
"""
...
from conciliacion.ui_helpers import (
    _NO_MAPEAR, _csv_download, _fmt_bool, _render_mapeo_parejas,
)
...
with st.sidebar:
    ...
    # Auto-detección de archivos locales
    _auto_listado_path = next(
        (p for p in local_xlsx if any(k in p.stem.lower() for k in ("listado", "colppy", "libro", "iva_compras"))),
        None,
    )
    _auto_arca_path = next(
        (p for p in local_xlsx if any(k in p.stem.lower() for k in ("arca", "comprobantes", "mis_comp"))),
        None,
    )

    file_listado = st.file_uploader("Listado IVA Compras", type=["xls", "xlsx", "csv"])
    if _auto_listado_path and not file_listado and not st.session_state.get("dismiss_auto_listado"):
        _c1, _c2 = st.columns([5, 1])
        with _c1:
            st.caption(f"Local detectado: `{_auto_listado_path.name}`")
        with _c2:
            if st.button("✕", key="btn_dismiss_listado", help="Ignorar archivo local"):
                st.session_state["dismiss_auto_listado"] = True
                for _k in ["ml_cols", "ml_sug", "ml_conf", "ml_formato", "left_fingerprints"]:
                    st.session_state.pop(_k, None)
        auto_listado = _auto_listado_path
    else:
        auto_listado = None

    file_arca = st.file_uploader("Comprobantes ARCA", type=["xls", "xlsx", "csv"])
    if _auto_arca_path and not file_arca and not st.session_state.get("dismiss_auto_arca"):
        _c1, _c2 = st.columns([5, 1])
        with _c1:
            st.caption(f"Local detectado: `{_auto_arca_path.name}`")
        with _c2:
            if st.button("✕", key="btn_dismiss_arca", help="Ignorar archivo local"):
                st.session_state["dismiss_auto_arca"] = True
                for _k in ["ma_cols", "ma_sug", "ma_conf", "right_fingerprints"]:
                    st.session_state.pop(_k, None)
        ...
        if _cols:
            ...
            try:
                ...
                if _hr is not None:
                    ...
                    st.session_state[_fp_key] = {k: v.to_dict() for k, v in _fps.items()}
                if hasattr(_fsrc, "seek"):
                    _fsrc.seek(0)
            except Exception:
                pass
            ...
            if _fmt_ml in ("libro", "libro_bim"):
                st.success(
                    f"📖 Libro IVA detectado — {len(_cols)} columnas. "
                    "Mapeo **automático**."
                )
            else:
                st.success(f"{_label} listo — {len(_cols)} cols.", icon="✅")
        else:
            st.session_state.pop(f"{_kp}_cols", None)
            st.session_state.pop("ml_formato", None)
            st.warning(f"Sin columnas en {_label}. Verificá el formato.")
        if hasattr(_fsrc, "seek"):
            _fsrc.seek(0)
    ...
    tolerancia = st.number_input(
        ...
        help="Diferencia máxima aceptable para considerar un importe como coincidente.",
    )
    ...
    with st.expander("🕓 Historial", expanded=False):
        prev_s1, prev_s2, prev_s3, prev_meta = (
            _prev_s1_g, _prev_s2_g, _prev_s3_g, _prev_meta_g
        )
        if prev_s1 is not None:
            fecha_str = str(prev_meta.get("fecha_proceso", ""))[:16].replace("T", " ")
            st.caption(f"Último: {fecha_str}  |  ±${float(prev_meta.get('tolerancia', 0)):.2f}")
            cargar_prev = st.button("Cargar último resultado", use_container_width=True)
        else:
            cargar_prev = False
            st.caption("Sin resultados guardados.")
        ...
        if historicos:
            sel_hist = st.selectbox(
                ...
                format_func=lambda h: "— Ninguno —" if h is None
                    else f"{h['label']} ({h['conc']}/{h['n']} conc.)",
                key="sel_hist_key",
            )
        else:
            sel_hist = None
        ...
        try:
            ...
            for _xtra in _raw_extras:
                ...
                if _cl and _cl != _NO_MAPEAR and _ca and _ca != _NO_MAPEAR:
                    _extra_cols.append({"label": _lbl or f"Extra_{len(_extra_cols)+1}", "col_l": _cl, "col_a": _ca})
            ...
            if df_listado is not None:
                _proc_status.update(label="Leyendo Comprobantes ARCA...")
            ...
            print(f"[ERROR] {_tb.format_exc()}", file=sys.stderr)
...
if st.session_state.get("loaded"):
    ...
    _kpi_cuit = (
        f'<div class="kpi or"><div class="n">{n_cuit_dif}</div><div class="l">CUIT difiere</div></div>'
        if n_cuit_dif else ""
    )
    st.markdown(f"""
    <div class="kpi-row">
    ...
    _n_reglas_tab = len(_reglas_cuit_global)
    tab1, tab2, tab3, tab4 = st.tabs([
        f"Conciliación ({n_l})",
        f"Solo Listado ({n_sl})",
        f"Solo ARCA ({n_sa})",
        f"Reglas CUIT ({_n_reglas_tab})" if _n_reglas_tab else "Reglas CUIT",
    ])
    ...
    with tab1:
        ...
        d1_disp = _fmt_bool(d1, ["Existe_en_ARCA", "Match_Neto", "Match_IVA", "Match_Total",
                                  "Conciliado", "ARCA_es_NC", "Match_OtrosTrib"])
            ...
            with pc2:
                ...
                if motivo_sel == "Otro (ver nota)":
                    motivo_custom = st.text_input(
                        ...
                        placeholder="Ej: retención IIBB no contemplada",
                    )
                ...
                nota = st.text_area(
                    ...
                    placeholder="Ej: diferencia de $0.12 por redondeo",
                )
                ...
                with btn_cols[0]:
                    if st.button("💾 Guardar corrección", type="primary", use_container_width=True,
                        ...
                        if guardar_como_regla and cuit_corr:
                            ...
                            if not ok_r:
                                st.error("No se pudo guardar la regla.")
    ...
    with tab2:
        ...
        with cc1:
            _csv_download(d2, f"⬇ CSV ({len(d2)} filas)",
    ...
    with tab3:
        ...
        cc1, _ = st.columns([2, 5])
        with cc1:
            _csv_download(d3_disp, f"⬇ CSV ({len(d3)} filas)",
                          f"solo_arca_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")

    # ── Tab 4: Reglas CUIT ────────────────────────────────────────────────────
    with tab4:
        reglas = _reglas_cuit_global
        if not reglas:
            st.info(
                "No hay reglas guardadas. Podés crearlas corrigiendo un comprobante "
                "en la tab **Conciliación** y marcando *Guardar como regla*."
            )
        else:
            st.caption(
                f"{len(reglas)} regla(s) activas — se aplican automáticamente "
                "a futuros comprobantes del mismo CUIT."
            )
            rows_reglas = []
            for cuit_k, rule in reglas.items():
                rows_reglas.append({
                    "CUIT":         cuit_k,
                    "Razón Social": rule.get("razon_social", ""),
                    "Estado":       rule.get("estado_display", rule.get("estado", "")),
                    "Motivo":       rule.get("motivo", ""),
                })
            df_reglas = pd.DataFrame(rows_reglas)
            st.dataframe(df_reglas, use_container_width=True, hide_index=True,
                         height=min(400, 45 + 35 * len(df_reglas)))

            st.divider()
            st.markdown("**Eliminar una regla:**")
            cuit_del = st.selectbox(
                "Seleccionar CUIT:",
                options=[""] + list(reglas.keys()),
                format_func=lambda k: "— elegir —" if k == ""
                    else f"{reglas[k].get('razon_social', k)} ({k})",
                key="del_regla_sel",
            )
            if cuit_del and st.button("Eliminar regla", type="primary", key="btn_del_regla"):
                eliminar_regla(cuit_del)
                st.success(f"Regla para CUIT {cuit_del} eliminada.")
                st.rerun()
    ...
    with c1:
        st.markdown("""
<div style="background:#252526;border-left:3px solid #007acc;border-radius:4px;
            padding:1.2rem 1.5rem;margin-bottom:1rem">
<h4 style="margin:0 0 .7rem;color:#9cdcfe">Pasos para conciliar</h4>

**1 · Archivos** — en el panel izquierdo cargá:
- Listado IVA Compras desde Colppy (`.xls` / `.xlsx`)
- Mis Comprobantes Recibidos desde ARCA (`.xlsx`)

**2 · Procesar** — presioná **▶ Procesar**

**3 · Resultados** — revisá KPIs, exportá a Excel o descargá CSV filtrado
</div>
""", unsafe_allow_html=True)

    with c2:
        st.markdown("""
<div style="background:#252526;border-left:3px solid #4ec9b0;border-radius:4px;
            padding:1.2rem 1.5rem;margin-bottom:1rem">
<h4 style="margin:0 0 .7rem;color:#4ec9b0">Funcionalidades</h4>

- NC en ARCA → montos en **negativo** automáticamente
- Facturas de **exterior** (FCC-A/B/C) discriminadas por Origen
- **Reglas CUIT** — correcciones que se aplican en cada procesamiento
- **Historial** de snapshots para comparar períodos
- **Emparejamiento inteligente** de columnas con detección semántica
</div>
""", unsafe_allow_html=True)
    ...
    historicos = _historicos_global
    if prev_s1 is not None:
        st.info("Hay un resultado previo disponible. Usá **Historial** en el sidebar.")
    if historicos:
        st.info(f"Hay {len(historicos)} snapshot(s) histórico(s) en el historial.")
