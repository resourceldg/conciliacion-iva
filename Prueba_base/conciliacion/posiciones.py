...
# ── Leer F.2051 IVA (DDJJ ARCA) ──────────────────────────────────────────────

def _parse_num_arca(s: str) -> float | None:
    """Parse formato argentino: '$\\xa03.463.580,40' → 3463580.40"""
...
def _parse_period_arca(s: str) -> tuple[int, int] | None:
    """Parse '202604' → (4, 2026)"""
...
def _parse_f2051_page(text: str) -> dict[tuple[int, int], dict[str, float | None]]:
    """
    ...
    def extract(pattern: str) -> float | None:
        mm = re.search(pattern + r"[^$\n]*\$\s*([\d.,]+)", text, re.IGNORECASE)
        return _parse_num_arca(mm.group(1)) if mm else None

    vals: dict[str, float | None] = {
        "debito_fiscal":     extract(r"d[eé]bito fiscal del per[ií]odo"),
        "credito_fiscal":    extract(r"cr[eé]dito fiscal del per[ií]odo"),
        "saldo_ld_anterior": extract(r"libre disponibilidad del per[ií]odo anterior"),
        "ret_perc_total":    extract(r"retenciones,\s*percepciones y pagos"),
        "saldo_final":       extract(r"libre disponibilidad a favor del contribuyente"),
    }
    return {period_key: vals}
...
def read_posiciones_pdf(
    ...
    """
    Lee el F.2051 IVA descargado de ARCA (1 página por declaración mensual).
    Retorna { (mes, año): { campo: valor } }.

    Campos disponibles: debito_fiscal, credito_fiscal, saldo_ld_anterior,
                        saldo_final, ret_perc_total.
    Nota: ret_perc_total = retenciones + percepciones + pagos a cuenta (combinados).
    """
    try:
        import pdfplumber
    ...
    with pdfplumber.open(source) as pdf:
        for page in pdf.pages:
            ...
            if "VALOR AGREGADO" in text.upper() and re.search(r"\b\d{6}\b", text):
                result.update(_parse_f2051_page(text))
